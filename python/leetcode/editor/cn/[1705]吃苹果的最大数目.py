...
class Solution(object):
    def eatenApples(self, apples, days):
        ...
        for day in range(1, num_days + 1):
            heapq.heappush(lst_eating, [days[day - 1] + day, apples[day - 1]])
            while lst_eating:
                now_apple = heapq.heappop(lst_eating)
                if now_apple[1] > 0 and now_apple[0] > day:
                    now_apple[1] -= 1
                    eaten += 1
                    heapq.heappush(lst_eating, now_apple)
                    break
...
apples = [2, 1, 10]
days = [2, 10, 1]
print(so.eatenApples(apples, days))
print("4 \n")

...
#  
#  Related Topics 几何 数组 数学 排序 滑动窗口 👍 44 👎 0
    ...
    def visiblePoints(self, points, angle, location):
        ...
        for x, y in points:
            if x==x1 and y==y1:
                ...
                lst_angle.append(m.atan2(y - y1, x - x1) * 180 / m.pi + 180)
        # 滑动窗体
        lst_angle.sort()
        length = len(lst_angle)
        max_window, window_length = 0, 0
        left, right = 0, 0
        while length:
            while (lst_angle[right] - lst_angle[left]) % 360 <= angle:
                window_length += 1
                right += 1
                right %= length
                if right == left:
...
angle = 1
location = [1,1]
print(so.visiblePoints(points, angle, location))
